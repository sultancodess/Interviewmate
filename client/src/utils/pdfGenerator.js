 ...
 * 
 * This utility generates professional PDF reports for interviews using jsPDF.
 * It creates a comprehensive report with:
 * - Header with candidate and interview information
 ...
 * @param {Object} interview - Interview data with evaluation
 * @returns {Blob} PDF blob
 */
export const generatePDF = async (interview) => {
  ...
  try {
    ...
    pdf.text(`Generated by InterviewMate AI`, margin, 32)
    ...
    pdf.setFontSize(20)
    pdf.setFont('helvetica', 'bold')
    pdf.text(`${overallScore}%`, margin + 20, yPosition + 18)
    
    pdf.setTextColor(0, 0, 0)
    pdf.setFontSize(12)
    pdf.text(`Grade: ${getPerformanceGrade(overallScore)}`, margin + 60, yPosition + 10)
    pdf.text('Overall Score', margin + 60, yPosition + 20)
    ...
    if (skillScores) {
      Object.entries(skillScores).forEach(([skill, score]) => {
        const skillLabel = SKILL_CATEGORY_LABELS[skill] || skill
        
        // Skill name
        pdf.setFontSize(10)
        pdf.setFont('helvetica', 'normal')
        pdf.text(skillLabel, margin, yPosition)
        ...
        // Fill bar
        pdf.setFillColor(59, 130, 246)
        pdf.rect(margin, yPosition + 2, fillWidth, barHeight, 'F')
    ...
    if (strengths && strengths.length > 0) {
      ...
      strengths.forEach((strength, index) => {
        const bulletPoint = `• ${strength}`
        yPosition = addWrappedText(bulletPoint, margin, yPosition, contentWidth, 10)
        yPosition += 3
      })
    ...
    if (badges && badges.length > 0) {
      ...
      pdf.setFontSize(16)
      pdf.setFont('helvetica', 'bold')
      pdf.text('Achievement Badges', margin, yPosition)
      yPosition += 10
    ...
    pdf.text('Generated by InterviewMate AI - Your AI-Powered Interview Practice Platform', margin, footerY)
    pdf.text(`Report generated on ${new Date().toLocaleDateString()}`, pageWidth - margin - 50, footerY)
 ...
 */
export const downloadPDF = async (interview, filename) => {
  try {
    ...
    link.href = url
    link.download = filename || `interview-report-${interview.candidateInfo.name}-${new Date().toISOString().split('T')[0]}.pdf`
